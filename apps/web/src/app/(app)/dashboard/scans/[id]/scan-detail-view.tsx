...
function reportFormatLabel(format: "pdf" | "csv"): string {
...
export default function ScanDetailView({ scanId }: Props) {
  ...
  const artifactsQuery = useQuery({
    ...trpc.reports.listArtifacts.queryOptions({ scanId }),
    enabled: scanQuery.data?.status === "completed",
  });

  function invalidateReportHistory() {
    void queryClient.invalidateQueries(trpc.reports.listArtifacts.queryFilter({ scanId }));
  }
              ...
              {artifactsQuery.data && (
                <>
                  <p className="text-xs text-muted-foreground">
                    Retained until {formatDate(artifactsQuery.data.snapshot.retainedUntil)}
                  </p>
                  {artifactsQuery.data.artifacts.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {artifactsQuery.data.artifacts.map((artifact) => (
                        <Card key={`${artifact.format}-${artifact.generatedAt.toString()}`}>
                          <CardContent className="flex items-center justify-between gap-4 pt-4">
                            <div className="space-y-1">
                              <div className="flex items-center gap-2">
                                <Badge variant="outline" className="px-2.5">
                                  {reportFormatLabel(artifact.format)}
                                </Badge>
                                <span className="text-sm">{formatDate(artifact.generatedAt)}</span>
                              </div>
                              <p className="text-xs text-muted-foreground">
                                {formatByteSize(artifact.byteSize)} · Generated by{" "}
